Shader "Unlit/_textureRendererWaterTest"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(g2f i) : SV_Target
            {
                ...
                float3 bump = UnpackNormal(i.pos);
                //float3 bump = UnpackNormal(tex2D(_MainTex, i.uv));
                float4 depthUV = i.grabPos;
				depthUV.xy = i.grabPos.xy + (bump.xy * _Shift);

                float surfDepth = UNITY_Z_0_FAR_FROM_CLIPSPACE(i.pos.z);
                ...
                float2 uvoffset = bump.xy * _Shift;
				float2 grabUV;
				grabUV = AlignWithGrabTexel((i.grabPos.xy + uvoffset * depthDiff) / i.grabPos.w);


                //float4 col1 = tex2D(_CameraOpaqueTexture, grabUV) * _Color;
				col = tex2D(_CameraOpaqueTexture, grabUV) * depthDiff * col;
                col /= 1.5;
                return col;
